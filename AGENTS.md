...
- Star나 Watch를 눌러 달라는 문구를 넣지 마라
- 첫 화면은 한국어다. 영어·일본어·중국어 간체·번체 소개는 `Docs/Readme/`에
  두고, 모든 소개 맨 위에서 다른 언어로 이동할 수 있게 한다.
  이 다국어 소개는 각 언어로 쓴다. 한국어 문장 순서를 그대로 옮기지 마라.
- 소개 페이지의 언어와 게임이 실제로 지원하는 언어를 구분해 안내한다.
