...
- 4층 하강 계단을 계단실 안(X -330 서쪽)으로 옮기면서 밤1 「관망 주머니」의
  체감이 바뀐다. 예전에는 바닥 서쪽 끝에서 72cm를 떨어져 반층 참(3.5F)에
  올라선 뒤 X -382까지 걸어갈 수 있었고, 지금은 계단을 한 단 내려간
  X -363에서 야간 포털이 걸린다. 형체(-445)는 여전히 포털 너머라 닿을 수
  없고 시야도 같은 방향이지만, 서 있는 자리가 참이 아니라 계단 위가 된다.
  실제 플레이로 좁게 느껴지면 `SetNightStairPocketEnabled`의 야간 포털
  X를 -440에서 -452쯤으로 밀면 계단 두 단까지 열린다.
