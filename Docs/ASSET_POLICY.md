...
| `/Game/Meshes/SK_ListenerCrawler`, `A_ListenerCrawler_{Crawl,Listen,Bang,Lunge}` | 직접 제작 (TRELLIS.2 생성물 → Blender 복셀 리메시·거리 웨이트 리깅·키프레임) | 프로젝트 소유 | 2026-09-11 | 원본 | `Content/SourceArt/Blender/SK_ListenerCrawler/`, `Scripts/blender/rig_crawler.py` | 위층 사람의 기는 몸. `Docs/BLENDER_PIPELINE.md` 「리깅된 인물」 |
| `Content/SourceArt/Reference/Listener/*` | Wikimedia Commons — 미 육군·해병대 낮은 포복 사진(공공 저작물), 살아 있는 조각상 사진(CC0 / CC BY-SA 2.0) | 참고 전용 | 2026-09-11 | 원본 | 파일명이 Commons 파일명 | 자세·석고 피막 참고. 게임 에셋으로 쓰지 않고 패키지에 들어가지 않는다 |
