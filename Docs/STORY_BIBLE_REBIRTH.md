...
> **보관 문서 — 2026-08-08 서사 전면 재구성으로 폐기.**
> 사용자 피드백(공포 구간 미흡, 스토리·개연성 부족, 퍼즐 난이도,
> 사운드 기획)에 따라 「4시 44분」의 서사를 폐기하고 완전 신규
> 스토리로 재구성했다. 현재 제작 정사는
> [STORY_BIBLE_MISSING_FLOOR.md](STORY_BIBLE_MISSING_FLOOR.md)
> (「없는 층」)이며, 이 문서와 충돌하면 새 정사가 우선한다.
>
> **전체 스포일러.**
>
> 이 문서는 `4시 44분`을 서사·공포·연출·음악·효과음·이미지·대화·텍스트까지
> 하나의 경험으로 다시 설계했던 과거 제작 기준이다. 실제 반영 상태는
> `IMPLEMENTATION_STATUS.md`를 따른다.
...
| 25 | 열쇠가 꽂혀 있다는 사실만으로는 고양이가 지운보다 먼저 닫힌 옥상 철문을 통과할 수 없음 | 처진 철문이 약 11cm를 남기고 문턱에 걸리는 실제 결함을 설정. 강만식이 자물쇠를 연 뒤 문틀 고리에 걸어 두고 열쇠를 꽂은 채 덜 닫고 내려가며, 고양이는 틈을 통과하고 지운은 뒤이어 문을 당겨 연다 | §2, §4.8, §8, §15, §17 |
...
이 문서의 `외부 사다리`는 수직 사다리가 아니라 물탱크 서쪽 서비스면에
고정된 45도 점검 계단을 뜻한다. 20cm 단차·20cm 진행의 오픈 트레드 18단,
폭 105cm이며 상부 착지부는 서쪽으로 치우친 점검구 테두리 직전에서 끝난다.
상단 두 번째 단에서 테두리까지 수평 거리는 약 107cm라, 오른발은 사고 단에
남고 왼발은 상단 단/착지부를 딛은 채 상체를 점검구 위로 기울일 수 있다.
...
| 처진 옥상 철문이 문턱에 걸려 약 11cm 열리고, 열린 자물쇠와 꽂힌 열쇠가 문틀에 남음 | 고양이가 지운보다 먼저 옥상에 들어가고 다시 나올 실제 통로가 생기며 지운은 잠금 조작 없이 문을 당김 | 문짝 아래 경첩 처짐·문턱 긁힘·열린 걸쇠·11cm 틈을 왕복한 앞발자국 |
...
P1과 P2가 같은 숫자 입력 규칙을 쓰는 것은 조작 학습을 재사용하기 위한
기능 계약이지, 같은 기계가 반복된다는 설정이 아니다. P1은 403호 선반 위의
둥근 디지털 알람 시계로 보이고 조사 문구는 `알람 시 조정 / 알람 분 조정 /
알람 시각 확인`이다. P2는 편의점 카운터의 금전함·감열지 배출구·3×4 숫자
키패드가 있는 POS이며 `원거래 시 조정 / 원거래 분 조정 / 원거래 시각 복원`을
사용한다. POS의 작은 숫자 키패드는 생활 소품의 외형이고, 플레이 입력은 더
크고 읽기 쉬운 세 버튼만 받는다. 따라서 플레이어는 새 조작법을 다시 외우지
않아도 공간과 물건의 현실성으로 두 퍼즐의 맥락을 즉시 구분한다.
...
| 옥상 열쇠·철문 | 열린 자물쇠가 문틀 고리에 걸리고 실린더에 열쇠가 꽂힌다. 처진 아래 경첩 때문에 문짝은 문턱에 걸려 약 11cm 열림 | 같은 상태 | 06:08 열쇠와 열린 자물쇠 회수, 현장 정리 뒤 06:10 무렵 문짝을 들어 밀어 완전히 닫아 잠금 | 고양이의 선행·탈출 통로와 지운의 진입 원인을 함께 보여 주는 환경 물증. 플레이어는 열쇠를 인벤토리에 넣거나 잠금을 조작하지 않음 |
...
문턱을 긁고, 자유단이 닫힌 문틀 평면에서 수평으로 약 `11cm` 열린 지점에
걸려 있다. 문짝 아래 전체가 11cm 뜬 상태가 아니며 하단은 문턱에 거의
붙어 있다. 문틀 고리에는 열린 자물쇠가 걸려 있고 관리 열쇠는 그
실린더에 꽂혀 있다. 자물쇠는 문짝과 문틀을 잇지 않는다. 고양이 앞발자국은 이 틈을 통과해 옥상으로 들어갔다가
...
자유단 `11cm` 상태로 돌아간다. 고양이는 자유단과 문설주 사이를 몸을
낮춰 굽어 통과할 수 있고, 사람은 그 틈이나 문 옆으로 우회할 수 없다.
플레이어는 문을 다시 당겨 계단실과 옥상을 계속 왕복할 수 있으며,
...
`핵심 소리 자막`은 지운의 생각 자막과 별도 레이어다. 켜면 CH02의 짧은
알람·라디오 단절, CH03의 후방 물소리·탱크 안쪽 긁힘, 양 엔딩의 안전 개방
절차와 암전 폴리, 봄 주방의 한 방울과 고양이 생존음을 대괄호 안에
표시한다. 꺼도 생각 자막·문서·퍼즐 UI는 사라지지 않는다.

크기는 85~125%, 안전 영역은 화면의 80~100% 범위에서 조정한다. 긴 문장은
실제 표시 글꼴의 글리프 폭을 재서 안전 영역 안에서 최대 두 줄로 나누고,
두 번째 줄도 넘치면 해석을 바꾸지 않는 지점에서 말줄임한다. 1280×720에서도
하단 조작 안내와 겹치지 않아야 한다. 연속 음향은 다음 소리가 시작되기 전에
앞 자막이 끝나며, 소리와 자막의 시작을 어긋나게 만들기 위해 대기열로 미루지
않는다. 이 두 설정값은 자막 켜짐 여부와 함께 재실행 뒤 유지한다.

소리 자막은 들린 방향·행동만 적고 `죽음`, `범인`, `정답`처럼 플레이어가
아직 증명하지 않은 해석을 덧붙이지 않는다. 같은 순간의 공포음 방향 파형과
함께 쓸 수 있지만, 어느 한쪽도 다른 쪽의 설정을 강제로 켜지 않는다.
...
- 카메라 물방울은 신규 CH03 기상에서 한 번만 사용한다. 저장 복원과 일반
  문서 캡처에서는 다시 재생하지 않는다.
- 물방울은 포스트프로세스 노멀 왜곡 또는 테두리·하이라이트·투과 알파를
  가진 절차 렌즈 프록시를 동등한 정식 표현으로 허용한다. 네이티브 HUD는
  물방울 위에서 선명해야 한다.
- `카메라 흔들림 감소`에서는 물방울의 화면 이동을 없애고 같은 한 번의
  출현·소멸만 유지한다.
...
- `04:31 원승인`은 `해온카드 / 체크카드 원승인 / 2024/07/26(금) 04:31 /
  새벽24 무영로점 / 승인 완료`를 같은 검은 폰 화면의 알림 카드로 표시한다.
  종이 질감·A4 비율·영수증 프린터 표현을 섞지 않는다.
- 종이·영수증·폰의 닫기 안내는 마지막 입력 장치에 맞춘 `E` 또는 `A` 하나만
  표시한다. 두 장치 키를 한 줄에 함께 쓰지 않는다.
...
|---|---|---|
| P0 | CH01 선택 상품 교체·결제 수단, CH02 재구매 제거, 실제 전화/이웃 반응, P1/P2 월드 시·분 입력과 3진실 중 2개 C3 합류를 연결했다. 04:31 원승인은 종이 패널과 분리한 전용 폰 알림 화면과 720p~1440p 경계 계약도 갖췄다. 무화면 P1→P2/P2→P1/P1 생략/P2 생략/양쪽 생략의 독립 프로세스 종단과 P1/P2 오답 중단 저장·재실행은 통과했으나 단말 최종 아트·실제 화면·초견 사람 왕복은 미검증 | 방 조사·편의점·P1·P2 생략/역순/왕복을 실제 입력으로 허용하고 C1~C3 진실 상태로만 합류 |
| P0 | CH01 `04:31`·CH02 `04:44`, A/B/C 영수증·봉지·결제 수단, 04:33 음용·재밀봉과 04:43 사다리 앞 배치는 같은 저장 상태와 실제 프롭으로 연결됐다. 고양이 선택도 `병뚜껑+먼저 귀가 / 병뚜껑+기다림 / 종이컵`에 따라 남은 용기 또는 젖은 원형 흔적을 복원한다. CH01 임시 흔적은 장 전환 때 정리하고 CH02가 저장 선택에서 표시·무충돌·비상호작용 단일 흔적을 다시 만들며, 지나감은 0개로 유지한다. 종이컵 종단과 다섯 저장 분기의 CH01·CH02 물리 검증은 구현됐지만 최종 렌더링·최신 바이너리 10프로세스 실행은 미승인이다 | 허용된 실행 환경에서 S5 세 고양이 용기 분기를 저장 전·후로 대조하고, CH01→CH03의 구매 프로필·결제 수단·봉지·병마개·고양이 선택이 같은 프롭 상태로 읽히는지 최종 화면 승인 |
...
| P0 | P3 절차·7개 저장 경계·서비스함 원본·IP65 인계표·미래 수사철·생략 경로는 코드와 정적 계약에 연결됐고 실제 UE 종료/별도 프로세스 재실행 14건을 통과했다. 최종 메시와 사람 초견 가독성은 미검증 | 정식 메시에서 90/150/210초 힌트, 투명 블리드관, 0kPa 두 틱, 오답 브라운아웃, 해결/생략 과실 합류를 S3 실제 입력으로 검증 |
| P0 | P5의 11개 조사 노드·15개 원자료·네 직접식·신원 보조식·관찰/확정 분리·저장 필드는 코드에 연결됐고 대표 네 경계의 실제 UE 저장·재실행을 통과했다. 최종 메시의 부분 훼손과 사람 실제 입력 가독성은 미검증 | 110개 첫 순서쌍·자기 재선택 11개·신원 보조식 6순서·관찰 저장 전수와 R2~R4 실제 입력에서 유효 프롬프트, `FocusedEvidence`, 세 긁힘을 검증 |
| P0 | 세이브 v3가 P1/P2 선택·압박, P3 압력·밸브, P5 포커스·긁힘·탱크와 엔딩 경계를 보존하고 REBIRTH를 태그 콜백보다 먼저 복원한다. CH01 첫 계단 암전 4프로세스, 고양이 선택 다섯 상태 10프로세스, CH02 기상 2상태 4프로세스, P5 4경계 8프로세스, P3 7경계 14프로세스, A/B 쓰기·커밋·재확인 6프로세스가 실제 패키징 Shipping의 `SaveGame`을 서로 넘겨 총 46개 독립 실행에서 통과했다. 정확한 영수증·저장 스냅샷 해시와 실행 전후 88파일 무변조도 함께 확인했다. 별도 맵 재진입 검증은 CH02 복도의 기존 12cm 낙하와 편의점 바닥 6cm 차이를 검출해 앵커를 수정했고 복도 최종값은 통과했으나, 편의점·CH03 세 지점의 최신 바이너리는 조직 정책 때문에 아직 미검증이다 | 같은 clean SHA에서 Shipping 46프로세스와 다섯 안전 앵커 10프로세스를 함께 통과시키고 S3·S4 실제 사람 종료/재실행을 검증 |
...
| P0 | A/B 공통 7월 31일 발견 상태·카드, 뚜껑·점검봉·안경의 실제 프롭 복원, 미래 문서·인체 회수, 안전 개방 다섯 음, A 마지막 진동과 B 가족 귀환·봄 주방 감각까지 그레이박스에 연결했다. 자동 경로는 공통 월드 상태와 안전음 순서도 검사하지만 최종 화면·청감은 미승인이다 | A/B 모두 최종 아트·오디오에서 암전 중 실제 상태 복원→안전 절차 폴리→공통 발견→각각 잔류/수용 감각을 화면·모노 폴드다운·자막 조합으로 승인. B는 정적 프롭+테이프·폰 폴리+최종 문구를 최소 정식 표현으로 허용 |
| P1 | 12행 접근성, 일시정지, 화면 설정의 키보드·게임패드 입력과 장치별 안내는 코드에 연결됐다. Shipping 전용 하네스가 비시뮬레이션 `InputKey` 11회와 실제 HUD 프레임 8개의 글리프 경계를 720p/900p/1080p/1440p에서 검사하도록 구현됐고, 일반 플레이에서는 계측 비용이 0이 되도록 플래그로 게이트했다. 세 타깃 컴파일과 88파일 재스테이지는 통과했지만 입력 프레임 순서를 고친 최신 EXE가 조직 Application Control로 실행 전에 차단돼 4해상도 PASS 영수증은 없다 | 허용된 동일 clean SHA 환경에서 전용 하네스 4해상도를 통과하고, 실물 게임패드·키보드·마우스로 메뉴 전환과 한글 경계를 사람 검증 |
| P1 | M0~M5 라우터와 폰 전용 UI, 난이도·힌트·접근성 기반은 코드에 연결했다. 오디오는 정적 계약 79개, 생산 생성기 8개의 PCM 런타임 프로브, Editor·Shipping 컴파일과 dirty Shipping A/B 실행을 통과했다. 최신 코드 전용 Shipping의 CH03 D3D12 5장과 렌즈 물방울 기본/모션 감소 4장도 통과해 기본 11.56px·감소 0px와 HUD 안전 영역을 확인했다. 실제 청감·사람 자유 입력·다중 디스플레이와 clean 완전 Cook은 미승인이다 | §10~13의 상태 기반 텍스트·오디오·이미지 계약을 실제 노트북·TV·헤드폰·모노와 접근성 조합으로 승인하고, clean SHA 런타임에서 `audio_synthesis` 무음·클리핑 0을 재확인 |

P3·P5의 정적 계약과 순수 상태 오라클, UE 5.8 그레이박스, 별도 프로세스
저장·재실행까지 통과했지만 위 표를 면제하지 않는다. 자동 검증은 원자료
문턱·상태 복원·기본 증명식의 회귀를 막는 단계이며, 최종 메시에서 사람이
직접 조작해 물증을 읽고 공포 리듬을 이해하는 완성 빌드 인증은 아니다.
