...
>
> 현재 빌드의 구현 기준은 `STORY_BIBLE.md`에 남아 있다. 이 문서는 기존
> 코드를 완료된 것으로 가장하지 않으며, 아래의 `§14 구현 전환표`를 따라
> 단계적으로 교체한다.
...
| # | 결함 | 조치 | 주요 반영 절 |
|---:|---|---|---|
| 1 | 지운이 산 생수의 행방이 문서 전체에서 사라짐 | 편의점 봉지를 3챕터 관통 모티프로 승격. 04:43 사다리 앞 하차 → 06:12 강만식이 쓰레기로 오인 회수 → 7월 30일 CCTV 대조로 재수색 확정 | §2, §5, §6, §7, §8 |
...
| 5 | 옥상 도달 직전에 문서 4장이 몰려 페이싱 붕괴 | 4층 2장 / 철문 앞 작업대 2장 / 철문 바깥 환경 안내 1개 / 철문 안쪽 클립보드 1장으로 분산. 옥상에 들어가기 전 읽을 종이는 두 장을 넘지 않음 | §8 |
...
| 20 | 바람이 무거운 철제 점검구 뚜껑을 편의상 닫는 것처럼 보임 | 접이식 지지쇠의 고정핀이 유실된 상태에서 관리인이 뚜껑을 뒤쪽 스토퍼에만 기대 둔 것으로 고정. 세 번째 돌풍은 흔들기만 하고, 사고 순간 더 강한 같은 방향 돌풍이 뚜껑을 수직선 안쪽으로 넘긴 뒤 중력으로 닫히게 함. 엔딩 B의 점검봉만 마지막 돌풍을 실제로 버틴다 | §2, §8, §15 |
...
| 30 | `봉지+벗겨진 발판`만으로 탱크 **안쪽** 추락까지 확정해 증거가 결론보다 약함 | 모든 `T_FALL` 경로에 안쪽 손바닥 쓸림 또는 상단에서 안쪽으로 기우는 기억 충돌을 필수로 포함. 봉지는 등반 동기만 보강 | §4.7, §8, §10 |
...
8. **결정적 증거를 지운 것도 악의가 아니라 안일함으로 만든다.**
   - 강만식은 옥상 바닥의 낯선 편의점 봉지를 조금 전 내려간 청소 업체
     기사가 놓고 간 작업 쓰레기로 단정하고 1층 재활용 마대에 넣는다.
   - 그는 증거를 인멸한 것이 아니라 청소를 한 것이다. 그리고 그 청소가
     닷새를 만든다.
...
| 7월 24일 21:40 | 어머니가 전화한다. 지운은 받지 않고 문자로만 답한다. |
...
| 03:56 | 청소 업체 기사가 부식된 연결 밸브를 발견하고 추가 작업을 거부한다. 기사는 점검구 뚜껑을 임시로 내려놓고 `현장 인계 03:58 / 관리인 강만식`에 서명을 받은 뒤 장비를 챙겨 내려간다. 옥상에 남는 젖은 건물 호스와 점검봉, 관리 열쇠는 업체 장비가 아니라 강만식의 관리 대상이다. |
| 04:01 | 건물주가 교체비 승인을 위해 수위와 부식 부위 사진을 요구한다. |
| 04:02~04:03 | 강만식은 혼자 점검구를 다시 열고, 촬영 각도를 확보하려고 받침 위 호스를 사다리 쪽으로 밀어 **끝을 점검구 테두리에 걸쳐 둔 채** 사진을 찍어 04:03에 보낸다. 접이식 지지쇠의 고정핀은 유실돼 체결할 수 없는데도, 철제 뚜껑을 수직보다 조금 뒤쪽의 스토퍼에만 기대 둔다. 그때 아래층에서 단수 항의 전화가 연달아 오자, 점검봉은 바닥에 놓고 내려간다. 아래 경첩이 처져 완전히 놓으면 문짝이 문턱에 걸리는 옥상 철문을 약 11cm 덜 닫고, 관리 열쇠도 바깥 자물쇠에 꽂은 채 내려간다. |
...
| 04:16~04:31 | 새벽24 무영로점에 다녀온다. 플레이어가 고른 생수 브랜드·용량·수량으로 결제하고 봉지에 담아 나온다. 지갑을 챙겼다면 지갑 속 카드, 두고 왔다면 전날 작업 뒤 후드 주머니에 남아 있던 실물 체크카드를 쓴다. 어느 쪽이든 승인 시각은 04:31이며, 집의 폰에는 와이파이를 통해 승인 알림이 남는다. |
...
| 04:42 | 고양이는 처진 옥상 철문이 문턱에 걸려 남긴 약 11cm 틈으로 먼저 빠져나간다. 뒤따른 지운은 바깥 자물쇠에 꽂힌 관리 열쇠와 열린 틈을 보고 철문을 당긴다. 지운이 문짝을 통과할 만큼 여는 사이, 고양이가 탱크 받침을 가로지르며 젖은 호스를 밟는다. 테두리에 걸쳐 있던 호스 끝이 안으로 미끄러져 수면과 철판을 한 번 크게 치고, 바깥쪽 호스 몸통 무게에 끌려 도로 빠져나와 받침 위로 늘어진다. 문을 다 연 지운이 보는 것은 받침 반대편으로 사라지는 꼬리, 점검구 쪽에서 끝난 젖은 앞발자국, 아직 울리는 물소리의 잔향, 열린 점검구다. 그는 고양이가 빠졌다고 오인한다. 실제 고양이는 받침 반대편으로 뛰어내려 지운의 시야 밖에서 같은 철문 틈을 지나 계단으로 달아난다. |
| 04:43 | 지운은 혀를 두 번 차 고양이를 부르지만 대답이 없다. 물에 빠졌는지만 위에서 확인하려고 외부 사다리 앞에 편의점 봉지를 내려놓는다. 구조를 위해 탱크 안에 들어가려는 행동이 아니다. |
| **04:44** | 지운이 점검구를 내려다보려고 상체를 열린 테두리 위로 기울인다. 오른발이 딛고 있던 **상단에서 두 번째 외부 발판**의 젖은 고무 패드가 벗겨지고, 동시에 왼손이 젖은 테두리에서 미끄러진다. 이미 중심이 점검구 안쪽에 있어 몸이 앞으로 넘어간다. 안경은 바깥 난간 연결부에 걸리고, 지운은 예비 저수조 안으로 떨어지며 어깨와 관자놀이를 내부 사다리에 부딪힌다. 깊은 잔수와 좁은 원통 안에서 방향을 잃고 세 번 철판을 긁는다. 앞서 세 번 예고됐던 방향에서 더 강한 돌풍이 불어, 지지쇠 없이 뒤쪽 스토퍼에 기대 있던 뚜껑을 수직선 안쪽으로 넘긴다. 이후에는 중력이 철제 뚜껑을 끝까지 닫는다. 지운은 익사한다. |
...
| 06:08 | 강만식이 열쇠와 현장 물품을 회수하러 옥상에 올라온다. 받침 위에 늘어진 호스를 당겨 감고 바닥의 점검봉을 치우면서, 지상에서는 닫힌 뚜껑만 본다. 그는 바람에 닫힌 뚜껑을 자신이 내려놓았다고 잘못 기억하고, 사다리에 오르지 않는다. |
| 06:12 | 내려오면서 사다리 밑의 낯선 편의점 봉지를 본다. 03:58에 내려간 청소 업체 기사가 놓고 간 작업 쓰레기라고 단정해, 묻거나 내용물을 확인하지 않고 1층 재활용 마대에 넣는다. 공동현관 CCTV에는 강만식이 계단 마지막 단을 내려와 같은 형태의 봉지를 마대에 넣는 전 과정이 남는다. 그는 그 뒤 옥상문을 잠근다. 악의는 없다. 그리고 이 성급한 청소가 닷새를 만든다. |
| 07:02 | 어머니가 “지운아”라고 보낸다. |
| 11:41 | “전화 좀 받아.” |
| 7월 27일 오전 | 정막례가 “사람 떨어지는 소리 같았다”고 1층 관리 연락함에 적어 넣는다. 강만식은 전날 아침과 똑같이 닫힌 뚜껑을 지상에서만 보고, 사다리에 오르지 않은 채 “고양이로 추정, 옥상 잠금 확인”이라고 답한다. |
| 7월 27일 저녁 | 지운의 어머니가 경찰에 실종 신고를 한다. 강만식은 자신이 현장을 다시 열었다는 사실을 숨기려고 “7월 26일 04:10 예비 저수조 배수·점검구 잠금 완료, 내부 비어 있음”이라는 허위 확인서를 만든다. |
| 7월 28일 | 경찰이 404호, 공용 복도, 5층, 옥상 바닥까지 1차 수색한다. 옥상은 이미 치워져 있어 호스도 봉지도 없다. 점검구 윗면은 지상에서 보이지 않고, 급수계통과 분리된 밀폐시설에는 추락 방지·밀폐공간 장비가 필요하다. 경찰은 강만식의 `내부 비어 있음` 확인서를 근거로 즉시 진입하지 않고, 원본 작업 기록과 출입 CCTV 추출을 요청한다. |
| 7월 29일 | 설치업체가 오래된 공동현관 CCTV 기록장치의 관리자 암호를 복구한다. 영상 화면의 시각 오버레이는 `TIME ERROR`지만, DVR 이벤트 파일에 저장된 서버 수신 시각과 프레임 순서는 살아 있어 설치업체가 경찰 추출본에 `04:38 / 06:12`를 분 단위로 복원한다. 가족은 병원과 근무지를 계속 확인하고, 정막례는 5층 계단참에 물 한 그릇을 놓는다. |
| 7월 30일 | 복구된 공동현관 로비 영상에서 두 장면이 나온다. `04:38` 봉지를 들고 귀가한 지운은 주출입구로 다시 나오지 않고, `06:12` 강만식은 같은 형태의 봉지를 들고 계단을 내려와 재활용 마대에 넣는다. 재활용은 매주 수요일 수거라 아직 남아 있다. 마대에서 그 봉지와 생수병이 회수되고, 품목이 04:31 결제 내역과 일치한다. 402호 윤성희의 `05:50 계단실 문 열림` 진술, 청소 업체의 `03:58 현장 인계`, 건물주가 받은 `04:03 열린 점검구 사진`, 강만식의 `04:10 배수 완료` 확인서가 모두 어긋난다. 경찰·소방은 이미 04:00으로 잡혀 있던 업체 재방문에 맞춰 다음 날 저수조를 함께 확인하기로 한다. 어머니는 22:14에 “내일 새벽에 경찰이 옥상 물탱크를 다시 본대. 거기 있으면, 조금만 기다려.”라고 보낸다. |
| **7월 31일 04:00** | 경찰·소방과 청소 업체가 안전 장비를 갖추고 사다리에 오른다. 점검구 바깥 난간 연결부에 걸린 검은 뿔테 안경과 빈 잠금 확인란을 확인하고 닫힌 점검구를 연다. 지운이 발견되어 가족에게 돌아간다. |
...
| 건물주의 사진 요구로 강만식이 04:02 다시 개방, 04:03 사진 전송 | 열린 점검구·테두리에 걸친 호스·꽂힌 열쇠가 남음 | 건물주 업무 대화와 개방 사진 메타데이터 |
...
| 지운이 철문을 당겨 여는 동안 호스가 이미 되돌아옴 | 그가 본 것은 꼬리·물소리·열린 점검구 셋뿐이라 오인이 성립 | 들어온/나간 고양이 발자국, 받침 위로 늘어진 호스 |
| 지운이 고양이가 빠졌는지 **확인하려고** 외부 사다리에 오름 | 상체를 점검구 위로 기울인 순간 상단에서 두 번째 발판 패드와 젖은 테두리 손이 함께 미끄러져 안쪽으로 추락 | 받침 아래 편의점 봉지, 상단까지 이어진 슬리퍼 자국, 테두리 손바닥 쓸림, 벗겨진 패드, 난간 안경 |
| 골목에서 두 번, 04:37에 세 번째로 예고된 건물 사이 돌풍 + 고정핀이 유실된 뚜껑 지지쇠 | 마지막 돌풍이 뚜껑을 수직선 안쪽으로 넘기고 중력이 닫아 지상에서 내부가 보이지 않음 | 잎·입간판·배수 체인, 04:37 뚜껑 흔들림, 빈 지지쇠 고리, 사고음 K |
| 강만식이 06:08 받침 위 호스만 회수하고 사다리에 오르지 않음 | 현장이 정리되어 1차 수색에서 이상이 보이지 않음 | 회수된 열쇠, 비어 있는 원본 측정·잠금 기록 |
| 강만식이 06:12 봉지를 쓰레기로 알고 1층 마대에 버림 | CCTV의 귀가 장면과 옥상 체류를 지상에서 즉시 연결할 물증이 현장에서 사라짐. 난간 안경은 사다리에 오르기 전에는 보이지 않음 | CH02 로비 마대의 같은 브랜드 생수병, 06:12 하강 영상 |
| 7월 27일 허위 `04:10 내부 비어 있음` 확인서 | 장비가 필요한 탱크 내부가 1차 수색에서 보류 | CH02 민원 답변, CH03 원본과 허위본 |
| 공동현관 CCTV 암호 복구와 원본 기록 대조 | 같은 로비 화각의 04:38 봉지 귀가와 06:12 봉지 하강·마대 투입이 맞물려 옥상 체류가 확정됨 | 두 영상 캡처, 마대 회수 품목과 04:31 결제 내역 일치 |
| 402호의 05:50 계단실 문 열림 진술 | 04:44 이후 옥상 동선이 열려 있었음이 확인됨 | 7월 30일 경찰 확인 요청서의 진술 요약 |
| 이미 예정된 7월 31일 04:00 재방문에 경찰·소방이 합류 | 안전 장비로 점검구를 열고 지운을 발견 | 철문의 재방문 안내문, 선택 전 업체 회신, 엔딩 B 합동 확인 카드 |
| 엔딩 B 직전 실제 사고 상태가 다시 겹침 | 점검봉 선택이 과거를 바꾸지 않음 | 닫힌 뚜껑·바닥 점검봉·난간 안경 0.8초 |
...
| 시기 | 놓인 것 | 마음 |
|---|---|---|
| 7월 27일 | 문지방의 소금 줄, 밥 한 공기, 수직으로 꽂은 숟가락 | 무언가 떨어져 죽었다고 느낀 노인의 방어와 배웅 |
| 7월 29일 | 5층 계단참의 맑은 물 한 그릇과 쪽지 | 쫓는 것을 그만두고 **기다리기로 바꾼** 마음 |

CH02 현관에서 **소금 줄은 이미 끊어져 있다.** 누군가 밟아 지운 것이
아니라 그녀가 손으로 흩은 것이다. 물그릇은 소금 줄 안쪽이 아니라
바깥에 놓여 있다. 이 두 배치가 “쫓다가 마음을 바꿨다”를 대사 없이
읽히게 한다. 밥과 숟가락은 이때 이미 치워져 있고, 마른 자국만 남는다.
...
- 06:08의 첫 오판은 자기 기억을 믿은 안일함, **06:12에 봉지를 치운 것은
  악의가 아니라 습관**, 7월 27일의 첫 거짓말은 책임을 인정하기 싫은
  공포, 허위 확인서는 자기보호에서 나온다.
- 그가 이야기에서 저지른 가장 치명적인 행동은 거짓말이 아니라 **청소**다.
  이 게임은 나쁜 사람이 아니라 성의 없는 확인이 사람을 늦게 찾게 만든다는
  이야기다.
- 직접 등장하거나 말하지 않는다.
...
- 오른쪽 귀에 작은 V자 흠이 있고, 높은음 뒤 낮은음이 붙는 쉰 두 음의
  울음이 있다. CH03의 대체 생존 단서도 이 두 특징 중 하나로 동일 개체임을
  확인시킨다.
...
| 사고 진실 1~3개 확보 | 0.60→0.82 | 긁힘이 낮아지고 가까워짐 |
| 탱크 조기 개방 | 0.88 | 정체를 모르는 몸; 10초 뒤 이동 가능 수준으로 감압 |
| `T_IDENTITY` 확정·C5 | 1.0 | 자기 발견, 어느 경로든 같은 최고점 |
| 엔딩 | 0.2 | 부정 또는 애도 |
...
3. 플레이어가 루프 안에서 물건을 옮겨도 현실의 7월 26일 이후 핵심
   ...
   프롭·생각 자막만 바꾼다.
4. 지운은 실제로 7월 31일 발견됐다. 엔딩 B가 발견을 만들어 내는 것이
   아니다.
5. 엔딩 A는 지운의 기억이 발견 이후로 넘어가지 못한 상태다.
...
3. **하나의 퍼즐은 하나의 진실만 증명한다.**
   퍼즐 하나에 여러 반전을 몰아넣지 않는다.
...
| P5 | CH03 옥상 | **연결** — 서로 맞물리는 두 물증으로 사고를 재구성한다 | 고양이가 밟은 호스가 물을 쳤고, 고양이는 빠져나왔으며 지운만 젖은 발판에서 추락했다 | 단서가 쌓일수록 탱크 긁힘이 가까워짐 |
...
확정할수록 긁힘이 가까워지고, C5의 모든 진실이 연결되는 순간
압박음 전체가 멎는다. 정답이 가까울수록 더 무서워지는 마지막 추론이다.
...
| P5 | 가장 가까운 미확인 진실에 따라 `나간 흔적이 있어.` / `물소리와 자국이 안 맞아.` / `발자국이 여기서 끊겼어.` / `내가 산 물이… 여기 있어.` 중 하나 | 현재 위치에서 가장 가까운 대체 물증에만 앞발·철판 물소리·젖은 고무·비닐 스침·안경 링 중 하나가 남 | `나간 흔적, 물을 친 것, 내려놓은 것, 끊긴 움직임, 탱크 속 모습을 연결하자` |
...
| 실제 결제는 04:31에 이루어짐 | 생수 브랜드·용량·수량, 진열대 조사 순서 |
...
이 약속은 실제로 지켜진다. 아홉 개 진실에는 각각 두 개 이상의 확보
경로가 있고, 어떤 퍼즐도 필수가 아니며, 어떤 문도 순서를 강제하지
않는다. 지키지 못할 약속을 하지 않는 것이 지킬 수 있는 약속을 크게
말하는 것보다 낫다.
...
| CH03 | 404호, 침수 복도, 서비스함, 반복 계단, 옥상, 사다리, 탱크 | P3/P4/P5 순서와 무관하게 방↔서비스함↔계단↔옥상 왕복 |
...
| `T_ALARM_0510` | 실제 알람은 05:10 | P1 시계 / 수험 플래너+05:10 알람 패턴 |
...
| `T_NEGLIGENCE` | 작업 종료·잠금 기록이 거짓 | P3 빈 측정값+04:03 개방 사진 / 03:58 인계+04:03 사진 / 원본+허위 확인서 |
| `T_CAT_SAFE` | 고양이는 탱크에서 나옴 | 들어온·나간 발자국 / 아래층의 쉰 두 음 울음+오른쪽 귀 V자 흠 |
| `T_HOSE_CAUSE` | 물을 친 것은 고양이가 밟은 호스 | 호스 위 압착 앞발+커플링 철판 충격 자국+테두리 안쪽 마찰 흔적 / 압착 앞발+되돌아온 호스+물·금속 복합음 기억 |
| `T_FALL` | 지운은 상단에서 두 번째 외부 발판에서 안쪽으로 추락 | 상단까지 이어진 슬리퍼 자국+젖은 발판+테두리 손바닥 쓸림 / 사다리 밑 편의점 봉지+벗겨진 고무 패드+안쪽 3cm 기억 충돌 / 테두리 손바닥 쓸림+내부 사다리 충돌 기억 |
| `T_IDENTITY` | 탱크 안의 몸은 지운 | 휘어진 안경+수색 전단의 복장+탱크 속 복장 / 403호와 탱크의 같은 등·왼쪽 소매 검은 실 세 땀+왼쪽 뒤축이 닳은 삼선 슬리퍼 |
| `T_FOUND_0731` | 발견은 이미 예정되고 실제로 일어남 | 업체 재방문 회신 / 철문의 재방문 안내문 / 어머니의 7월 30일 재수색 문자 / 경찰 확인 요청서 |
...
| CH03 문서 | 경찰 확인 요청서의 회수 품목과 06:12 하강 영상 | `T_FOUND_0731` — 재수색이 결정된 실제 근거 |
...
|---|---|---|---|
| C1 편의점 결제 | `T_NEED_WATER` 또는 편의점 직접 도착 | 방 조사 순서, 계단/엘리베이터, 상품 선택 | 어떤 상품이든 04:31 결제 프로필 저장 |
...
| C5 마지막 선택 | C3의 세 상태 전부+`T_CAT_SAFE`+`T_HOSE_CAUSE`+`T_FALL`+`T_IDENTITY`+`T_NEGLIGENCE`+`T_FOUND_0731` | 빠진 C3 진실의 CH03 회수, 물증 순서, 탱크 조기 개방, P3 또는 문서 증명 | 동일한 시간·사고·수색 진실과 공간 안의 선택으로 합류 |
| C6 발견 이후 | 선택 B+실제 상태 복원 | 앞선 선택·조사량과 무관 | 고정된 7월 31일 합동 발견을 보여 줌 |

C3는 세 상태 중 둘만으로 챕터를 넘길 수 있지만, 남은 하나를 `NarrativeDebt`
목록에 넣어 CH03의 가까운 대체 경로에서 회수한다. C5는 C3의 세 상태와
마지막 여섯 상태가 모두 모이는 유일한 합류점이며, 각 상태에는 두 가지
이상의 확보 경로가 있다. 선택 프롬프트가 아직 나오지 않을 때
...
| P3 전체 | 옥상 원본 인계표+04:03 사진+허위 확인서 | 배관 조작은 생략되지만 `T_NEGLIGENCE` 확보 가능 |
| 옥상 인계표·확인서 | P3 빈 측정값+업무 대화 출력물+401호 민원 | 다시 내려가 서비스함을 조사하거나 세 출처를 조합 |
| 나간 고양이 발자국 | 아래층의 쉰 두 음 울음+난간 아래로 보이는 오른쪽 귀 V자 흠 | `T_CAT_SAFE` 대체 경로 |
| 검은 안경 | 403호의 등+회색 후드+흰 면티+왼쪽 뒤축이 닳은 삼선 슬리퍼 / 수색 전단 복장+탱크 속 복장 | `T_IDENTITY` 대체 경로 |
| CH02 재활용 마대 | CH03 옥상 봉지+경찰 확인 요청서의 회수 품목 | 같은 봉지를 뒤에서 다시 만나게 함 |
| 옥상 봉지 | 경찰 확인 요청서의 `06:12 관리인 하강` 영상 요약 | 실물을 못 봤어도 문서로 같은 사실에 도달 |

단서 이월은 플레이어 앞에 문서를 순간이동시키는 기능이 아니다.
영수증은 지갑, 종료 기록은 서비스함, 인계표는 옥상 작업대, 소리는 해당
...
2L 두 병을 고른 플레이어에게 이 동작은 **여섯 번째 반복**이고, 500mL를 고른
...
RecomputeConvergence():
    ...
    C5Facts = all(
        ...
        T_FALL, T_IDENTITY, T_FOUND_0731
    )
    FinalChoiceAvailable =
        C5Facts and IdentityRecognized and PlayerLookingInsideTank
...
| 생수·편의점 봉지 | 편의점에서 생성, 손에 듦 | 외부 사다리 밑 | 06:12 강만식→1층 페트 마대→7월 30일 경찰 회수 | CH02 마대와 CH03 사고 직후 옥상 상태가 겹쳐 보임 |
...
| 옥상 열쇠·철문 | 바깥 자물쇠에 열쇠가 꽂히고, 처진 아래 경첩 때문에 문짝이 문턱에 걸려 약 11cm 열림 | 같은 상태 | 06:08 강만식이 열쇠를 회수하고 문짝을 들어 밀어 완전히 닫아 잠금 | 고양이의 선행·탈출 통로와 지운의 진입 원인을 함께 보여 주는 환경 물증. 열쇠는 인벤토리에 넣지 않음 |
...
| 건물 사이 돌풍 | 골목에서 같은 방향으로 두 번, 잎·입간판·배수 체인이 반응 | 첫 돌풍 누락, 두 번째에 잎만 역행 | 옥상의 바람과 사고 당시 뚜껑 폐쇄 |
...
| **서비스함** | 4층 복도 벽, 잠긴 철제함. 네 라벨과 작은 계통도만 읽힘 | 그대로 잠김. 단수 공지에 예비조 분리 문구가 붙음 | 수압으로 열려 있고 실제로 조작 가능 |
...
| 지운은 어떻게 떨어졌는가? | 젖은 슬리퍼 자국, 사고음 K, 세 번의 긁힘 | P5의 상단에서 끊긴 사람 자국·상단 두 번째 발판·테두리 손바닥 쓸림·난간 안경 | 탱크의 내부 사다리와 지운의 등 |
| 그가 산 물은 어디로 갔는가? | CH01 내내 손에 들려 있고 상호작용마다 내려놓던 봉지 | CH02 마대의 같은 브랜드 생수병, CH03 사다리 밑의 봉지 | 06:12 하강 영상과 회수 품목이 재수색을 결정 |
...
| 엔딩 B가 과거를 고치는가? | 선택 전에 읽는 7월 31일 업체 재방문 문서와 철문 안내문 | 점검봉은 기억 속 뚜껑만 붙들고, 전환 직전 닫힌 뚜껑·바닥의 점검봉·난간 안경으로 실제 상태가 복원됨 | 같은 발견일의 경찰·소방·업체 합동 확인을 보여 주고 “찾는 사람이 있었다”로 종결 |
...
이 네 물건은 나중에 신원을 증명하므로 착용 과정을 생략하지 않는다.
별도 드론은 없다.
...
이 두 장면은 카메라를 고정하거나 자막을 붙이지 않는다. 대신 현관문
상호작용에 옷을 걷어 입고 슬리퍼에 발을 넣는 1.2초 1인칭 손·발 동작을
포함해, 플레이어가 시선을 돌려도 화면 아래에서 회색 소매와 삼선이 한 번
보이게 한다. 왼쪽 지퍼를 올리는 순간 소매 안쪽의 터진 솔기를 검은 실로
세 번 꿰맨 자국이 0.6초 보인다. 별도 확대나 효과음은 붙이지 않는다.
**CH02의 삼각 대조가 성립하려면 여기서 아래 고리에 한 벌만
있던 상태와 착용 동작을 반드시 보여 줘야 한다.** 고리가 하나뿐이거나
...
전화 두 통 사이에도, 전화 뒤에도 지운의 답은 언제나 문자다. 이 사실을
설명하는 문장은 어디에도 없다. 플레이어가 스스로 알아보게 둔다.
...
같은 함이 수압으로 열려 있을 때, 처음 보는 장치가 아니라 “늘 지나치던
그 함”이어야 한다. 사고를 일으킨 물리적 원인은 전부 정상 형태로 먼저
심는다는 원칙(§16-22)이 배관에도 적용된다.
...
| 9.0초 | 같은 방향의 두 번째 짧은 돌풍. 전선과 빌라 옥상 배수 체인이 함께 흔들린다. |
| 12.0초 | 배달 오토바이가 통과한다. |
...
| 예시 프로필 | 상품 | 손 | 봉지 실루엣 |
|---|---|---|---|
| A | 새벽샘물 500mL × 2 | 한 손 | 얕고 넓게 선다. 병 두 개가 서로 기댄다 |
| B | 한강수 1L × 1 | 한 손 | 좁고 높다. 걸을 때 다리에 한 번씩 닿는다 |
| C | 맑은산 2L × 2 | **양손** | 손잡이가 늘어나 한 손은 손잡이, 다른 손은 봉지 밑을 받친다. 바닥에서는 넓고 낮게 선다 |

브랜드·용량·수량은 자유지만 생수를 하나 이상 스캔해야 결제가 가능하다.
...
- 프로필 A·B는 한 손이므로 하차 동작이 없다. 대신 걸을 때의 무게
  소리와 병 부딪힘이 다르다.
- 04:33, 골목에서 지운이 한 병을 따 몇 모금 마신다. 자동 재생이며
...
배수 체인**이 두 번째 돌풍에 흔들리고 있다. 원인은 즉시 보이고,
스팅도 드론도 없다.
...
404호 문을 처음 열면 CH01과 같은 1.2초 착의 동작을 반복해 후드집업을
입고 슬리퍼를 신는다. 두 번째 반복에는 자막이나 강조음을 붙이지 않는다.
플레이어가 스스로 “어제와 같은 외출 준비”로 받아들이게 한다.

착의 뒤 404호 아래 고리와 신발턱은 비고, 화면 아래에는 회색 소매가
남는다. 이 상태에서만 403호의 같은 후드집업과 빈 아래 고리의 모순이
성립한다. 저장 후 재개하거나 404호로 되돌아와도 착의 애니메이션과
프롭을 다시 생성하지 않는다.
...
| 돌풍과 잎 | 첫 돌풍은 오지 않고, 두 번째 돌풍 때 잎 세 장만 반대로 구른다. |
...
삼선 슬리퍼를 신는다. 세 번째 반복도 1.2초이며 대사는 없다. 이 장에서
...
안쪽의 문서 두 장이 드러난다. 종이가 물속에 그대로 놓여 있다가 멀쩡히
읽히는 연출은 하지 않는다.
...
##### 문서 2 — 건물주와의 업무 대화 출력물

원본 기록 아래에 프린터 용지 한 장이 붙어 있다. 사람이 쓴 기록이
비어 있는 것과 대조적으로, 기계가 남긴 시각은 정확하다.

```text
04:01  건물주
교체 승인 전에 수위와 부식 부위 사진 보내 주세요.
...
`업체가 닫아 넘겼는데… 다시 연 건 관리인이야.`

두 장을 함께 보면 `T_NEGLIGENCE`가 성립한다. 한 장만 봐도 상태는
확보되는 것이 아니라 출처 하나만 등록된다. 빈 종료 기록은 “안 적었다”를,
04:03 사진은 “다시 열었다”를 각각 말할 뿐, 둘 중 하나만으로 허위 완료를
확정할 수 없다. 두 번째 장을 대조했을 때만 상태가 성립하며, 플레이어가
이미 옥상의 원본+허위 확인서로 같은 진실을 증명했다면 여기서는 4음절
이하의 확인 또는 무대사로 끝낸다.
P3를 생략했다면 같은 상태를 옥상의 원본 인계표와 허위 확인서 조합으로
확보할 수 있다.
...
| 문서 | 놓인 곳 | 증명하는 것 |
|---|---|---|
| 원본 작업 종료 기록 | **4층 서비스함** (P3 정답 뒤) | 측정값과 안전 확인이 전부 비어 있음 |
| 건물주 업무 대화 출력물 | **4층 서비스함** (같은 자리) | 04:01 요구, 04:03 재개방 |
| 업체 원본 인계표 | 옥상 철문 앞 작업대 | 03:58에 업체가 닫아 넘겼음 |
| 7월 27일 허위 확인서 | 같은 작업대, 인계표 위에 겹침 | `04:10 내부 비어 있음`이 거짓 |
| 7월 30일 경찰 확인 요청서 | **철문 안쪽 면의 방수 클립보드** | 봉지 영상·회수 품목·주민 진술이 재수색을 만들었음. 옥상에 들어간 뒤 선택 조사 |
| 재방문 안내문 | **철문에 테이프로 붙음** | 7월 31일 04:00이 이미 예정돼 있음 |

4층 두 장, 철문 앞 작업대 두 장, 철문 바깥 환경 텍스트 하나, 철문 안쪽
클립보드 한 장이다. 플레이어가 옥상에 들어가기 전에 마주치는 읽기 문서는
작업대 두 장뿐이며, 경찰 공문은 프리던 하늘을 먼저 본 뒤 돌아볼 때
발견한다. 이보다 늘리지 않는다.
5층 계단참에는 문서를 두지 않는다. 그 구간은 공포를 슬픔으로 바꾸는
자리이며, 종이가 한 장이라도 들어가면 톤이 무너진다.
...
| 허위 확인서 | **유일하게 깔끔한 `04:10`** | 나중에 책상에서 지어냈다 |
...
세 번째 줄의 분 자리는 물에 번져 읽히지 않는다. 잔수는 숫자 대신
탱크 단면에 그은 짧은 선 하나다. 서명 두 개만 또렷하다.

`업체가 닫아 넘긴 거였어.`

체크 세 칸이 모두 비어 있는 것은 03:58 시점의 사실이다. 강만식은
06:08~06:12에 호스와 열쇠를 실제로 회수했지만, 그것은 그가
`04:10 완료`라고 적은 시각보다 두 시간 뒤의 일이다. **비어 있는 칸은
게으름의 증거가 아니라 시각이 맞지 않는다는 증거다.**

#### 7월 27일 확인서

인계표 위에 겹쳐 붙어 있다. 다른 종이보다 하얗고 반듯하다.
...
`비었다고 썼다. 확인한 칸은 하나도 없는데.`

#### 7월 30일 경찰 확인 요청서

철문 **안쪽 면**의 투명 방수 클립보드에 한 장만 끼워져 있다. 문을 당겨
옥상에 들어간 뒤 돌아봐야 보이며, 문 바깥의 작업대에서는 읽히지 않는다.
위탁 관리업체로 온 공문이며, 강만식이 다음 날 재방문을 준비하면서 출입
확인용으로 철문 안쪽에 걸어 둔 것이다.

```text
[현장 확인 요청]

수신  무영주택관리 / 달빛빌라 담당 강만식
건    실종자 한지운(29) 소재 확인
...
이 한 장이 봉지 모티프의 마지막 회수이며, `T_FOUND_0731`의 가장 강한
출처다. 동시에 플레이어에게 **사고의 진실은 하나도 알려 주지 않는다.**
호스도, 발판도, 탱크 안도 여기에는 없다. 이 문서가 증명하는 것은
“사람들이 그를 찾고 있었고, 내일 새벽 여기를 연다”뿐이다.

작업대의 종이는 업체 원본 인계표와 7월 27일 허위 확인서, **두 장이
전부**다. 경찰 공문을 작업대에 복제하지 않는다.

#### 철문의 재방문 안내문

문서 패널이 아니라 **환경 텍스트**다. 철문을 열려고 다가서면 눈높이에
걸리고, 읽는 데 3초면 된다. 조사 프롬프트도 전면 패널도 쓰지 않는다.
...
이 날짜 정보는 선택 전에 `T_FOUND_0731`의 어느 경로로든 확보돼야 한다.
엔딩 B의 7월 31일은 플레이어가 새로 만든 미래가 아니라, 이미 예정된
실제 시간의 회수다.

안내문을 못 보고 지나쳤고 `T_FOUND_0731`만 남았다면, 옥상에 90초
머물렀을 때 철문 아래로 업체 회신 봉투가 바람에 밀려온다. 플레이어가
직접 펼칠 수 있으며 자동으로 내용을 읽거나 선택을 막는 팝업은
사용하지 않는다.
...
3. 기본적으로 고양이·차량·풀벌레가 완전히 없음. 단, `T_CAT_SAFE`
   대체 경로가 필요할 때만 아래 주차장의 고양이 큐 1회 허용
4. 바람과 탱크 로프만 남음
...
| 호스 끝의 철판 충격 자국 + 점검구 테두리 안쪽의 짧은 젖은 마찰 흔적 | 사다리를 올라야 보임 | 두 흔적을 함께 봐야 “한 번 들어갔다 나왔다”가 성립 |
| 옥상문에서 외부 사다리 **상단까지** 이어지는 사람 슬리퍼 자국 | 하단은 받침 그늘, 상단은 사다리를 올라야 보임 | 아래 자국과 상단에서 끊긴 자국을 함께 봐야 한다 |
| 젖은 **상단에서 두 번째** 외부 발판과 벗겨진 고무 패드 | 또렷함 | 상단에서 보이는 테두리 손바닥 쓸림과 함께 읽어야 안쪽 추락이 성립 |
...
5. 지운이 철문을 다 열었을 때 호스는 이미 받침 위에 있다. 그가 본 것은
   꼬리·물소리의 잔향·열린 점검구 셋뿐이다.
...
3. 04:44의 더 강한 같은 방향 돌풍이 뚜껑을 `90°` 안쪽으로 넘긴다.
   그 뒤 폐쇄를 끝내는 힘은 바람이 아니라 뚜껑 자체의 무게다.
4. 닫힌 뒤 래치는 자동으로 걸리지 않는다. 내부 사다리는 있지만, 지운은
   추락하면서 그 사다리에 관자놀이와 왼쪽 어깨를 먼저 부딪혀 방향 감각과
   팔의 힘을 잃는다. 세 번 벽을 긁어 사다리를 찾으려다 수면 아래에서
   의식을 잃는다. **뚜껑은 익사의 유일한 원인이 아니라 발견을 늦춘
   원인**이며, 죽음은 추락 충격·깊은 물·좁은 원통이 겹친 결과다.
...
열린 채 유지된다. 마지막 돌풍이 시작될 때 점검봉을 빈 지지쇠 고리와
받침 홈 사이에 끼운 경우에만 `rod_wedged`가 되어 닫히지 않는다.

#### 봉지가 여기 있는 이유

`T_FALL`의 가장 직관적인 물증이다. 점검구를 확인하려면 양손이 필요했고,
그래서 그는 짐을 내려놓았다. 프로필 C를 골랐던 플레이어에게 이 동작은
CH01 내내 봐 온 습관의 마지막 반복이다.
...
이 봉지는 현실에서는 06:12에 강만식이 쓰레기로 알고 치웠고, 7월 30일
1층 마대에서 회수되어 재수색의 결정적 근거가 된다. CH03 옥상에 보이는
것은 04:44에 멈춘 기억 속의 상태다.

별도 스팅 없이 플레이어가 직접 보게 한다.

나간 발자국을 지나쳤거나 빗물에 번진 변형에서는 옥상 난간 아래의
주차장 센서등이 켜진다. CH01과 같은 쉰 두 음 울음 뒤 고양이가 빛을
가로지르고, 오른쪽 귀의 V자 흠이 1초 보인다. 옥상에서 떨어진 유령상이
아니라 현재 시점의 살아 있는 고양이임을 자동차 아래 그림자·정상 속도·
센서등으로 확인시킨다. 이 장면은 `T_CAT_SAFE`가 없을 때 한 번만 발생한다.
...
| 진실 | 1차 물증 | 대체·보강 물증 | 확정 생각 |
|---|---|---|---|
| `T_CAT_SAFE` | 옥상문으로 들어왔다가 다시 나간 고양이 발자국 | 아래층의 쉰 두 음 울음과 난간 아래로 보이는 오른쪽 귀 V자 흠 | `고양이는 나갔어.` |
| `T_HOSE_CAUSE` | 호스 위에 눌린 앞발 자국 | 커플링의 철판 충격 자국+점검구 테두리 안쪽의 젖은 마찰 흔적 / 받침 위로 되돌아온 호스+물·금속 복합음 기억 | `물을 친 건… 호스였고.` |
| `T_FALL` | 상단에서 끊긴 슬리퍼 자국·젖은 상단 두 번째 발판·테두리 손바닥 쓸림 | 사다리 밑 봉지+벗겨진 고무 패드+상단에서 안쪽으로 3cm 기우는 기억 충돌 / 손바닥 쓸림+내부 사다리 충돌 기억 | `보려고 몸을 기울이다… 안으로 미끄러졌어.` |
| `T_IDENTITY` | 점검구 바깥의 휘어진 검은 뿔테 안경·수색 전단 복장·탱크 속 복장 | 403호와 탱크의 같은 등·왼쪽 소매 검은 실 세 땀 / 왼쪽 뒤축이 똑같이 닳은 삼선 슬리퍼 | `저 안에 있던 게, 나였어.` |

한 오브젝트를 눌렀다고 즉시 진실이 완성되는 것이 아니라, 서로 맞물리는
두 단서를 보거나 이미 확보한 기억과 대조했을 때 상태가 확정된다. 어느
경로든 진실당 최소 두 출처가 남으며, 출처 하나를 놓쳐도 대체 경로가
활성화된다.
...
- 호스 위 앞발 자국을 기억한 채 테두리 마찰 흔적과 호스 끝 충격 자국을
  같은 선으로 잇는다.
- 사다리 밑 봉지를 기억한 채 상단 발판을 보면 빈손으로 올라간 동선과
  테두리 손바닥 쓸림을 연결한다.
- 403호의 등과 왼쪽 소매 세 땀을 기억한 채 탱크 속 등·같은 수선,
  또는 수색 전단 복장을 기억한 채 휘어진 안경과 탱크 속 복장을 대조한다.
...
| 안경을 누르면 `T_IDENTITY` 획득 | 안경과 **수색 전단의 복장+탱크 속 복장**, 또는 **403호와 탱크의 등·같은 소매 수선+왼쪽 뒤축 마모**가 대조돼야 확정 |
...
- 플레이어의 상체는 이미 점검구 높이에 있고, 카메라가 안쪽으로 3cm
  기울었다가 즉시 복구
- 검정 프레임 0.08초
- 물속에서 세 번 철판 긁는 소리가 겹침

강제 추락은 하지 않는다.

같은 높이의 점검구 테두리에는 왼쪽에서 안쪽으로 길게 쓸린 손바닥 물자국이
남아 있다. 발판 하나만으로 추락 방향을 설명하지 않고, `상단 발판 패드 +
안쪽으로 향한 손자국 + 난간 안경` 세 물증으로 몸의 중심이 점검구 안쪽에
있었다는 것을 확정한다.
...
모든 C5 진실 상태가 모이는 순간, 플레이어의 위치와 무관하게 바람과
긁힘이 동시에 멎는다. 옥상 밖이라면 탱크 쪽 생활음이 사라졌다는 것을
먼저 느끼고, 돌아와 내부를 다시 바라볼 때 다음 생각이 이어진다.

`안에서 세 번 긁은 건… 나였어.`

C5 전에는 뚜껑이 열려 있어도 마지막 선택 상호작용이 나타나지 않는다.
...
조기 개방했던 플레이어는 C5를 충족하고 같은 내부를 다시 바라볼 때에만
위 두 문장을 듣는다. 먼저 알아본 플레이어에게 `누구지…`를 강제로
재생하지 않는다.

### 장면 15. 공간 안의 선택

메뉴를 띄우지 않는다. C5의 모든 진실이 확보되고 탱크 속 신원을
완전히 인식한 뒤에만 손잡이와 점검봉이 선택 가능한 오브젝트가 된다.
그전에는 둘 다 배경 프롭으로 남고, 플레이어의 이동과 조사는 계속 열린다.
...
6. 암전 2.55초: 아주 멀리, 빈방의 책상 위에서 **폰이 한 번 진동한다.**
...
두 엔딩의 조작은 대칭이다. 손잡이를 잡거나, 봉을 끼운다. 그런데 B에는
사운드 몽타주와 에필로그가 붙고 A는 카드로 끝난다. 이 볼륨 차이를
그대로 두면 플레이어는 B를 정답으로, A를 “안 본 엔딩”으로 인식하고,
`§15`가 요구하는 “좋은/나쁜이 아니라 머묾/받아들임” 판단이 무너진다.

그래서 A에는 정보가 아니라 **상실**을 하나 준다.
...
이 선택을 실패라고 부르지 않는다.
플레이어는 공포 때문에 덮을 수 있다. 그러나 덮은 순간 기억은 다시
마지막 심부름으로 돌아간다. 현실에서는 7월 31일의 발견이 이미
일어났지만, 지운은 아직 그 시간에 도달하지 못한다.
...
- 안전 하네스 버클 두 번
- 서로 다른 무게의 사다리 발소리 두 세트
- 카라비너 세 개가 난간에 걸리는 소리
- 실제 점검구 뚜껑을 여는 금속음
- 404호 도어락
- 서랍을 여는 소리
- 책 네 권을 상자에 넣는 소리
...
| CH03 업무 대화 출력물 | `업체가 닫아 넘겼는데… 다시 연 건 관리인이야.` |
| CH03 수색 전단 | `내 이름인데.` |
| CH03 5층 수색 번호표 | `(무대사)` |
...
| CH03 허위 확인서 | `비었다고 썼다. 확인한 칸은 하나도 없는데.` |
| CH03 경찰 공문 | `봉지를… 들고 내려간 사람이 있었어.` |
| CH03 철문 안내문 | `(무대사)` |
| CH03 고양이·호스 | `고양이는 나갔어. 물을 친 건… 호스였고.` |
...
| `T_NEGLIGENCE` 미확보 | `끝났다는 작업 기록을 확인하자` | P3 또는 옥상 문서 |
| `T_FOUND_0731` 미확보 | `7월 31일에 예정된 일을 확인하자` | 폰·철문 안내문·업체 회신 |
| `T_CAT_SAFE` 미확보 | `옥상에서 나간 흔적을 찾자` | 발자국 또는 아래층 울음 |
| `T_HOSE_CAUSE` 미확보 | `밟힌 호스와 물에 닿은 흔적을 대조하자` | 호스 위 압착 앞발+커플링 충격·테두리 안쪽 마찰, 또는 압착 앞발+물·금속 복합음 기억 |
| `T_FALL` 미확보 | `사다리에서 끊긴 움직임과 기운 방향을 확인하자` | 발자국+발판+안쪽 손자국 또는 봉지+고무 패드+안쪽 기억 충돌 |
| `T_IDENTITY` 미확보 | `403호와 탱크 안의 모습을 대조하자` | 휘어진 안경+수색 전단 복장 또는 같은 등·왼쪽 소매 세 땀+뒤축이 닳은 왼쪽 슬리퍼 |
...
- 옥상에서 먼저 탱크를 열고 내려가도 M4의 완전 침묵은 옥상에만 남는다.
  실내로 돌아오면 원래 생활 베드가 복구되고, C5가 완성되는 순간에만
  전 구역에서 탱크 방향 바람이 사라진다.
...
| 계단 | 콘크리트 잔향 | 배경 | 첨벙, 표지 철판, 세 번 긁힘 |
| 옥상 | 도시 원경, 바람, 로프 | 등장하지 않음 | 생활 소리 페이드아웃, 탱크 공진 |
...
| 같은 트리거 | 6.5초 | 승용차 |
| 같은 트리거 | 9.0초 | 두 번째 돌풍·전선·옥상 배수 체인 |
| 승용차 통과 | 5.5초 | 오토바이 |
| CH01 편의점 입장 | 4~7초 | 백룸 철제 카트의 낮은 `쿵`, 이어 바퀴·박스 소리 |
| CH01 결제 후 출입문 방향 전환 | 즉시 | 유리 반사의 위쪽 움직임. 나가서 올려다보면 배수 체인 실제 흔들림 |
...
| 엔딩 B 실제 상태 복원 | 0.8초 | 뚜껑 닫힘·점검봉 바닥 낙하·안경 링을 서로 다른 거리로 배치 |
...
| P5 옥상 | 바람·로프 | 첫 시선과 첫·세 번째 새 사고 진실 확정에 따라 긁힘 위치를 탱크 상단→중단→수면으로 이동 | C5의 모든 진실이 연결되면 바람·긁힘을 끄고 로프 공진만 남김 |
...
**구도:** 천장에서 최대 35cm 내려온 셔터가 화면 위를 누른다. 지갑 결제라면 빈 카운터
위 `04:31` 영수증과 미리 놓인 `04:44` 영수증, `원거래 시각 __:__`
POS가 삼각 구도를 만든다. 주머니 체크카드 분기라면 첫 꼭짓점을 기록
보기에 남은 후드 주머니 영수증 또는 방의 폰 `04:31 승인` 화면으로
교체한다. 상품 행은 `PurchaseProfile`과 일치해야 한다.
...
|---|---|---|---|
| P0 | 내러티브 라우터와 C1~C6 합류점 | 기존 챕터·엔딩 상태 머신 | 11개 진실 상태, 출처 배열, `NarrativeDebt`, 거리 기반 목표 선택, 합류 조건 평가기 |
...
| P0 | 유실된 뚜껑 고정핀과 엔딩 B 점검봉 지지 상태 분리 | 기존 점검구 뚜껑·바람·점검봉 | 힌지·뒤쪽 스토퍼·빈 지지쇠 고리, `LidSupportState`, 04:37 흔들림/04:44 폐쇄, `rod_wedged`에서만 마지막 돌풍 차단 |
...
| P0 | P3 두 유입 차단·블리드 압력 해제 후 본배수와 문서 증명 경로 | 기존 침수·배관 프롭 활용 | 소구경 블리드가 포함된 실제 계통도, 4밸브 상태 머신, 압력계·수위 상한, 빈 측정값+04:03 사진 교차 증명, 생략 시 옥상 원본+허위본 조합 |
...
| **P0** | **편의점 봉지 관통 모티프** | 기존 생수·픽업·POS | 손에 들리는 봉지 프롭, 프로필별 실루엣, 프로필 C의 자동 하차·재파지, CH02 마대 병 동기화, CH03 옥상 봉지, 경찰 문서의 회수 품목 |
...
| **P0** | **호스 인과 수정** | 기존 옥상 프롭 | 점검구 테두리 걸침 상태, 밟힘 시 1회 낙하·복귀, 받침 위 최종 위치, 테두리 안쪽 마찰 흔적 데칼 |
...
| **P0** | **문서 재배치** | 기존 읽기 노트 | 4층 서비스함 2장, 철문 앞 작업대 2장, 철문 바깥 환경 텍스트 1개, 철문 안쪽 방수 클립보드의 경찰 공문 1장. `작업표 읽기` 단일 프롬프트 폐기 |
...
| **P0** | **옥상 철문의 실제 고양이 통로** | 기존 옥상 철문·열쇠·발자국 | 처진 아래 경첩, 문턱 긁힘, 11cm 걸림 상태, 고양이 왕복 충돌체, 사람은 당긴 뒤 통과, 손을 놓으면 11cm로 복귀, 열쇠 획득 금지 |
...
#### 현재 코드와의 격차 — 아직 구버전인 항목

`STORY_BIBLE.md` 기준으로 구현된 부분이 남아 있다. 아래는 REBIRTH로
넘어올 때 **반드시 교체**해야 하며, 문서만 고치고 코드를 두면 두 개의
정사가 공존한다.

| 현재 코드 | REBIRTH 기준 |
|---|---|
| 엔딩 B가 `안경을 물 위에 놓고 뚜껑을 열어 둔다` | 점검봉으로 기억 속 뚜껑을 고정하고, 안경은 난간에 그대로 둔다 |
| 발견일 `2024년 8월 16일` | `2024년 7월 31일 04:00` 경찰·소방·업체 합동 확인 |
| 탱크 개방 게이트가 안경 조사 필수 | 탱크는 옥상 도달 직후부터 열 수 있고, 게이트는 **마지막 선택**에만 적용 |
| 04:38의 세 번 긁힘이 사고의 원인 | 긁힘은 원인이 아니라 04:44에 지운이 남긴 마지막 소리의 기억 |
| CH03 방을 나가기 전 폰 조사 강제 | 어떤 조사도 강제하지 않는다. 진실은 상태로 판정한다 |
| 손전등 배터리 소모 | 소모 없음. 밝기 변화는 압박 연출 전용 |

### 최소 서사·플레이 완성 경로

예산이 제한되면 다음 열다섯 개를 한 묶음으로 먼저 구현한다.

1. 11개 진실 상태·출처 배열·`NarrativeDebt`·C1~C6 합류 평가기
...
7. 03:58 업체 인계 / 04:03 강만식 재개방 / 06:12 봉지 회수 / 7월 31일
   합동 발견 기록
8. 미래 문자·미전송 초안·401호 민원·수색 전단의 상호 대체
9. P3 `유입 차단→압력 해제→6초 0압력 확인→본배수` 절차, 옥상 원본 문서의 대체 증명
10. P4 물길 판단, 즉시 옥상 진입, 양방향 귀환
11. P5 복수 물증 경로, 순서 적응형 긁힘, 탱크 조기 개방·재방문
12. **호스 테두리 걸침 인과**, 고정핀 없는 뚜껑 물리와 옥상 물증의 부분 훼손
13. 엔딩 B의 기억 속 점검봉, 실제 상태 복원, 7월 31일 합동 발견
14. **엔딩 A의 마지막 진동**
15. 탱크 실루엣·수중 굴절·손전등 미소지 귀환·조기/완전 인식 이미지와 사운드 분기

이 열다섯 개는 따로 떼어 축소하지 않는다. 자유도 엔진 없이 퍼즐만
만들면 선형 진행으로 되돌아가고, 대체 단서 없이 문만 열면 플레이어가
진실을 모른 채 결말에 도착한다. **왕복 가능성, 복수 출처, 상태 기반
합류**가 시간의 진실·관리 부주의·사고 물증·엔딩 의미와 함께 구현되어야
최소 완성 단위다.
...
10. 엔딩 B가 발견을 새로 만든 것인가, 이미 일어난 발견을 지운이
    받아들인 것인가?
11. 지운이 산 생수는 어디로 갔는가?
12. 지운은 왜 어머니에게 전화 대신 문자로만 답했는가?
...
| 퍼즐 회피형 | P1·P2·P3을 시작하지 않고 문서·폰·환경음만 조사 | 대체 출처로 아홉 C5 진실 확보, 정답 UI 강제 없음 |
...
| 탱크 선행형 | 옥상 도착 즉시 탱크를 열고 내부를 본 뒤 내려감 | `누구지…`만 재생, 선택은 숨김, C5 뒤 완전 인식과 선택 개방 |
...
확보해야 한다. 어떤 경로도 같은 오브젝트를 강제로 만지게 할 필요는
없지만, 테스터는 위 핵심 질문 열두 개에 같은 사실로 답해야 한다.
...
1. 176cm 캡슐의 오른발을 상단에서 두 번째 외부 발판, 왼손을 젖은
   점검구 테두리, 상체 중심을 테두리 안쪽에 둔다.
2. 고무 패드 마찰과 왼손 지지를 같은 프레임에 해제한다.
3. 캡슐이 바깥 바닥이 아니라 점검구 안쪽으로 넘어가 내부 사다리의
   어깨·관자놀이 높이와 충돌하는지 확인한다.
4. 같은 메시에서 내부 높이 2.15m·잔수 1.80m·호스 8m·걸침 35cm·
   고양이 발 위치를 재생해 커플링이 수면과 철판을 한 번씩 친 뒤
   받침 위로 완전히 복귀하는지 확인한다.
5. 고정핀이 없는 상태에서 04:37 풍압은 `90°`를 넘지 않고, 04:44
   풍압만 수직선 안쪽으로 넘긴 뒤 중력으로 완전히 닫히는지 확인한다.
   점검봉을 고리와 받침 홈 사이에 끼운 `rod_wedged`에서는 같은 마지막
   풍압에도 닫히지 않아야 한다.
6. 고양이 충돌체가 처진 철문의 `11cm` 틈을 들어가고 나오는 방향으로
...
- 뚜껑 리허설 20회 중 04:37 조기 폐쇄 0회, 04:44 완전 폐쇄 20회,
  `rod_wedged` 상태 폐쇄 0회
...
- 8명 중 6명 이상이 엔딩을 “좋은/나쁜”보다
  “04:44에 머묾/발견 이후를 받아들임”으로 설명
- 8명 중 6명 이상이 두 엔딩의 실제 과거는 같고, 차이는 지운이
  04:44 이후를 받아들이는지에 있다고 설명
...
| 장별 착의 상태 | CH01~03 각 장에서 첫 퇴실 때만 1회 실행. 재입장·저장 후 재개·403호 왕복에서 후드와 슬리퍼가 복제되거나 다시 나타난 사례 0 | 해당 빌드 출시 차단. 장면 스크립트가 아니라 `OutfitEquippedByChapter` 저장·프롭 상태를 수정 |
...
| 문서 페이싱 | 옥상 철문을 처음 당기기 전에 전면 문서 패널로 읽는 종이 최대 2장. 경찰 공문은 철문 안쪽에서만 보이고 작업대 복제 0 | 문서를 삭제해 인과를 약화하지 말고 위치·가시 시점을 분리 |
...
처진 철문의 11cm 틈으로 먼저 빠져나간 고양이를 따라 지운이 문을 당긴다.
철문을 다 연 지운이 보는 것은 사라지는 꼬리, 물소리의 잔향, 열린
점검구뿐이라 고양이가 빠졌다고 오인한다. 사다리 앞에 봉지를 내려놓고
오르다 상체를 점검구 위로 기울인 순간 상단에서 두 번째 발판과 젖은
테두리 손이 함께 미끄러지고, 04:44에 잔수가 남은 탱크
안에서 세 번 철판을 긁고 죽는다. 마지막 돌풍이 고정핀 없는 뚜껑을
수직선 안쪽으로 넘기고, 이후에는 뚜껑 무게가 끝까지 닫는다.

한 시간 반쯤 뒤 강만식은 호스와 열쇠를 회수하면서 사다리에는 오르지 않고,
내려오는 길에 낯선 편의점 봉지를 청소 업체가 놓고 간 쓰레기로 단정해
1층 재활용 마대에 넣는다. 악의는 없다. 그리고 이 성급한 청소가 닷새를 만든다.
재활용 수거는 매주 수요일이라 7월 30일까지 봉지가 남는다.
...
0에 닿기까지 6초를 기다린 뒤 본배수를 열어, `배수 완료`라면 남아야 할
유입 차단·압력 해제 시각·0압력·배수량이 비어 있음을 증명할 수도 있고,
옥상의 원본
인계표와 허위 확인서를 조합할 수도 있다. 계단의 물줄기를 거슬러
올라가거나 즉시 옥상에 간 뒤 되돌아와 조사할 수도 있다.
...
진실이 모인 C5에서 같은 인식과 마지막 선택으로 합류한다.

마지막 선택은 메뉴가 아니다. 뚜껑을 닫으면 편의점 차임과 04:44 알람이
다시 시작되고, 암전 끝에서 빈방의 폰이 한 번 진동한 뒤 끊긴다. 화면은
켜지지 않는다. 그 문자는 영원히 읽히지 않는다.
...
7월 31일 04:00 경찰·소방과 청소 업체가 닫힌 저수조를 함께 열어 지운을
발견했다. 가족은 그를 데려가고, 어머니는 자기가 보낸 상자를 열흘
