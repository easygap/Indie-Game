...
| P0 | 세이브 v3가 P1/P2 선택·압박, P3 압력·밸브, P5 포커스·긁힘·탱크와 엔딩 경계를 보존하고 REBIRTH를 태그 콜백보다 먼저 복원한다. P1/P2 두 중단 상태·P5 4경계·P3 7경계·A/B 공통 발견은 실제 UE `SaveGame` 별도 프로세스 32건을 통과했다. CH01 첫 계단 암전은 전·후 4프로세스, 고양이 선택은 병뚜껑·종이컵·지나감 다섯 상태의 저장값과 CH01·CH02 양쪽 물리 흔적을 함께 복원하는 10프로세스 검증까지 구현했다. CH02 기상 완료 자동 저장도 활성화하고 `Loop.Started / AlarmStopped / Standing` 세 상태를 합격식으로 고정했으나 회사 PC Application Control 차단 뒤 추가돼 런타임 미승인이다. 별도 맵 재진입 검증은 CH02 복도의 기존 12cm 낙하와 편의점 바닥 6cm 차이를 검출해 앵커를 수정했고 복도 최종값은 통과했으나, 편의점·CH03 세 지점의 최종 바이너리도 같은 이유로 아직 미검증이다 | §4.7 세이브 표의 남은 필드, CH01 암전 원자성 4프로세스·고양이 선택 10프로세스와 다섯 안전 앵커 10프로세스를 허용된 clean 환경에서 통과시키고 S3·S4 실제 종료/재실행을 검증 |
