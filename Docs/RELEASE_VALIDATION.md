...
Saved/Validation/RebirthRelease/<UTC+PID>/
  ...
  PersistenceSpikes/
    ...
    SaveSnapshots/
      P3Write_0.sav ... EndingCommit_B.sav
...
| S2 11cm 문·고양이 | 최종 옥상 메시에서 문을 걸림 상태로 놓고 틈을 실측한다. 고양이 충돌체를 진입 20회·이탈 20회 통과시킨다. 사람 캡슐은 입력만으로 20회 접근한 뒤 문을 당겨 20회 통과하고 매번 손을 놓는다. | 간격 `11±1cm`, 고양이 40/40 통과, 데칼·몸 관통 0회. 사람은 당기기 전 0/20, 연 뒤 20/20 통과하고 매번 걸림 상태로 복귀한다. 문 옆 우회·내비게이션 탈출 0회다. |
...
2. 하네스의 Map Check가 `Prologue_Morning`에서 오류 0으로 끝나는지
   확인한다.
3. Development 빌드 뒤 `Run-Rebirth-PersistenceSpikes.ps1`이 고유
   `-UserDir`에서 P3 일곱 경계를 각각 별도 쓰기·읽기 프로세스로
   검증하고, A/B를 각각 선택 전 쓰기→공통 발견 커밋→재실행 검증으로
   확인한다. 쓰기와 공통 발견 커밋 직후의 `.sav` 복제본도 보존한다.
   상위 하네스는 20개 결과의 로그·세이브 경로와 SHA-256·커밋 SHA,
   읽기 뒤 원래 슬롯 삭제, 공통 prefix 1회성·분기 배타성 마커를 다시
   검사한다.
4. A/B 자동 런타임 로그가 `collision_route`, `audio_queue`, `p3_p5`,
   `savegame_v3`, 해당 엔딩과 `complete` 마커를 모두 갖는지 확인한다.
   A는 CH01 구매·결제·고양이 선택의 실제 이벤트 핸들러부터 CH02
   P1/P2·관리 민원과 CH03 인계까지 같은 세션에서 통과해야 한다. 이
   경로는 장별 착의 중복 0회와 정적 소매 세 땀을 확인한다. A/B 모두
   ...
   실제 문 통과 입력이나 사람 입력을 증명하지 않는다.
5. `FEASIBILITY.md`의 S1~S8을 위 수동 합격 사례대로 실제 종료·재실행까지
   수행한다. 각 사례에 무편집 영상, 시작·종료 세이브, 이벤트 로그와
   결과표를 남긴다.
6. R1~R4를 실제 입력으로 완주한다. R1, R2, R3, R4마다 재사용하지 않는
...
7. `STORY_BIBLE_REBIRTH.md` §15의 공간 왕복·사고 물리 회귀를 최종
   충돌체로 실행하고 공중 낙하·관통·진행 불능을 0으로 만든다.
8. 새 세이브와 v3 중단 세이브에서 종료·재실행을 확인한다. 에디터 세션
   안의 리셋만으로 저장 복원을 통과 처리하지 않는다.
9. 대표 저사양·권장 사양에서 실제 플레이 프레임과 메모리를 측정하고
