...
- 자동 UE 로그의 Ensure·Fatal·Unhandled Exception·Assertion과
  `Log*: Error:`는 기본적으로 실패다. 게임·프로젝트 진단의 자동 G3 일반
  허용목록은 비어 있다. 설치된 `5.8.0-55116800`에 한해 Epic Dataflow의
  `FDataflowToolNodeSnapshot::Date` 결정성 검사 두 줄은, 중간의 단일
  `Uninitialized script struct` Display 줄과 뒤따르는 엔진 초기화까지
  정확히 연속할 때만 알려진 엔진 시작 진단으로 분리한다. 한 줄이라도
  빠지거나 순서·위치·엔진 빌드가 달라지면 실패한다. 그 밖의 예외가
  필요하면 하네스에 정확한 서명과 허용 사유를 함께 추가하고, 같은 내용을
  매니페스트 승인 목록에도 기록한 새 SHA에서 다시 실행한다.
...
6. A/B 자동 런타임 로그가 `collision_route`, `audio_synthesis`, `audio_queue`,
   ...
   있으면 해당 자동 단계는 실패다. 게임·프로젝트 일반 허용목록은 0개이며,
   위 UE 5.8.0 Dataflow 시작 서명만 빌드·순서·개수가 모두 일치할 때 분리된다.
