...
| 색 | 크림/베이지 (원본 luma 0.772) | **세이지 회녹** (채택본) |
| 대비 요소 | 무늬 | 결과 광택차 |
| 실효 알베도 | 0.772 × 0.70 = **0.541** | 틴트로 같은 값에 맞춘다 |

### 채택: 아홉 장 중 4번 — 실측

후보를 세션 트랜스크립트에서 꺼내 `--rank`로 쟀다(전송본이라 원본과
해상도·압축이 다를 수 있다. luma와 pitch는 리샘플에 강하고, stipple과
seam은 참고값이다).

```
 #  file        size    luma  sdev  stipple  blob   seam h/v    pitch  /frame  notes
 1. cand01  1254x1254  0.820   7.0     0.62     2  0.36/0.18  57.89mm      28  ok
 2. cand09  1254x1254  0.617   3.4     0.92     3  0.75/0.61   7.89mm     209  ok
 3. cand07  1254x1254  0.591   3.5     1.03     2  0.34/0.49   7.89mm     209  ok
 4. cand04  1254x1254  0.654   6.5     1.23     3  0.24/0.41  13.16mm     125  ok  <- 채택
 ...
 9. cand02  1254x1254  0.821   5.2     1.76     3  1.27/0.35  17.11mm      96  seam
```

**결 굵기가 눈으로 본 것과 달랐다.** 4번을 「가는 결」이라고 했는데 실제로는
13.16mm로, 요청한 6mm의 두 배가 넘는다. 1번이 57.89mm(비드보드)라는 눈
판정만 맞았다.

그런데 **13.16mm 쪽이 오히려 옳다.** 1024px에 165cm면 결 하나가 8.2픽셀이라
밉 사슬을 견디고 방 건너편에서도 읽힌다. 7.89mm는 4.9픽셀, 내가 적어 보낸
6mm는 3.7픽셀로 둘 다 거리에서 먼저 사라진다. **6mm라는 사양이 1024 타일에
비해 너무 가늘었다** — 채택 판단이 사양보다 나았다.

stipple 1.23은 2026-08-14 승인 대역(0.39~1.37) 안이고, seam 0.24/0.41은
조건화 전인데도 출시 중인 벽지(0.88/1.04)보다 낫다.
...
### `detail_gain` 은 1.7 — 유추가 아니라 스윕

처음엔 먹지와 같은 2.0으로 두었다(「종이지 강판이 아니다」). 실제로 돌려
보니 틀렸다. 채택본은 stddev 6.5로 들어왔는데, 2026-08-14 배치가 1.86~3.78로
왔던 것의 두 배다 — 억제문을 쓰고도 결이 살아 있었다는 뜻이라 게인이 덜
필요하다.
...
참고로 2026-08-14 실적은 철제 계단 3.0, 옥상 방수 3.0, 석고 파편 2.4,
