...
if not exist "%PROJECT_ROOT%\Saved\Logs" (
    echo Could not create the REBIRTH greybox log directory.
    exit /b 1
)

if exist "%RUN_LOG%" del /q "%RUN_LOG%"
if exist "%RUN_LOG%" (
    echo Could not remove the previous REBIRTH greybox log.
    exit /b 1
)
...
if not exist "%RUN_LOG%" (
    echo REBIRTH greybox log was not created.
    exit /b 1
)

findstr /C:"REBIRTH_GREYBOX FAIL" "%RUN_LOG%" >nul
if not errorlevel 1 (
    echo REBIRTH greybox validation reported a failure. Check %RUN_LOG%.
    exit /b 1
)

findstr /C:"REBIRTH_GREYBOX PASS" "%RUN_LOG%" >nul
if errorlevel 1 (
    echo REBIRTH greybox validation did not report PASS. Check %RUN_LOG%.
    exit /b 1
)

echo REBIRTH greybox validation passed.
endlocal
