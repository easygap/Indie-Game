...
for name, title, lines in (
    ("PumpProcedure", "저수조 점검 메모", ["2019.03 / 관리실 보관", "", "넘친 날: 배수는 잠겨 있었고", "우회만 열려 있었음.", "", "펌프가 멎은 날: 배수만 열고 돌림.", "우회관에 물이 안 찼음.", "", "세척할 때는 자동 운전 금지.", "고장등이 켜지면 10초 대기.", "배관이 조용해지고 등이 꺼진 뒤", "다시 돌리세요."]),
...
# 생성 참조의 한글을 그대로 확대하지 않고, 정확한 글자로 저작한다.
im=Image.new('RGB',(768,1210),(214,215,210)); d=ImageDraw.Draw(im)
def label(text,x,y,size):
    f=ImageFont.truetype('C:/Windows/Fonts/malgun.ttf',size)
    d.text((x,y),text,font=f,fill=(28,31,30),anchor='mm')
label('이 송 펌 프',384,140,50)
for x,t in ((180,'전원'),(384,'운전'),(588,'고장')): label(t,x,497,31)
for x,y,t in ((293,719,'수동'),(384,687,'정지'),(476,719,'자동')): label(t,x,y,28)
label('저수조 세척용',384,1050,27)
im.save(OUT / 'PumpPanelFace.png')
